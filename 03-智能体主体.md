...
| 41 | **Tracing** | 分布式追踪 | 在Agent系统中记录每一步推理、工具调用、子任务执行的完整轨迹,用于调试和可观测性 |
| 42 | **LangSmith** | LangChain观测平台 | LangChain公司的LLM应用观测与评估平台,提供Tracing、评估、Prompt管理等功能,是LangGraph生态的默认可观测性工具 |
| 43 | **OpenTelemetry** | 开放可观测性标准 | 云原生计算基金会(CNCF)主导的开放可观测性标准,统一了Tracing、Metrics、Logs三类遥测数据的采集和导出 |
| 44 | **LiteLLM** | LLM代理库 | 开源的LLM代理库,提供统一接口调用100+种LLM(OpenAI/Anthropic/Gemini/Ollama等),让Agent框架不绑定单一模型供应商 |
| 45 | **RBAC** | 基于角色的访问控制 | 通过角色分配权限而非直接给用户授权,是企业级多租户Agent平台的标准安全模型 |
| 46 | **Pydantic** | Python数据验证库 | 基于类型注解自动进行数据验证和序列化,是FastAPI的核心依赖 |
| 47 | **in-context learning** | 上下文学习 | LLM无需微调,仅通过Prompt中的示例就能学会执行新任务的能力,是GPT-3时代发现的关键涌现能力 |
...
### 7.19 Agent 框架能力矩阵(Loop / Multi-Agent / Harness / Prompt)

> 本节横向对比 17 个框架在 4 项核心能力上的支持程度,帮助快速筛选。
>
> - **Agent Loop**:框架是否原生支持智能体循环(工具调用→观察→推理→行动→重复)?
> - **Multi-Agent 通信**:框架是否支持多个 Agent 之间通信协作?
> - **Testing Harness**:框架是否提供内置测试/评估框架?
> - **Prompt 管理**:框架是否有系统化的提示词模板管理?

#### 7.19.1 能力总览矩阵

| # | 框架 | Agent Loop | Multi-Agent 通信 | 测试工具 | 提示词管理 |
|---|------|-----------|-----------------|---------|-----------|
| 1 | LangChain | ✅ 是 | ⚠️ 部分 | ✅ 是 | ✅ 是 |
| 2 | LangGraph | ✅ 是 | ✅ 是 | ✅ 是 | ✅ 是 |
| 3 | AutoGPT | ✅ 是 | ❌ 否 | ❌ 否 | ⚠️ 部分 |
| 4 | BabyAGI | ✅ 是 | ❌ 否 | ❌ 否 | ❌ 否 |
| 5 | CrewAI | ✅ 是 | ✅ 是 | ⚠️ 部分 | ✅ 是 |
| 6 | AutoGen (MS) | ✅ 是 | ✅ 是 | ⚠️ 部分 | ⚠️ 部分 |
| 7 | AgentScope | ✅ 是 | ✅ 是 | ⚠️ 部分 | ⚠️ 部分 |
| 8 | MetaGPT | ✅ 是 | ✅ 是 | ❌ 否 | ✅ 是 |
| 9 | OpenAI Swarm | ✅ 是 | ✅ 是 | ❌ 否 | ⚠️ 部分 |
| 10 | LlamaIndex | ✅ 是 | ⚠️ 部分 | ⚠️ 部分 | ✅ 是 |
| 11 | PydanticAI | ✅ 是 | ⚠️ 部分 | ✅ 是 | ✅ 是 |
| 12 | smolagents | ✅ 是 | ⚠️ 部分 | ⚠️ 部分 | ⚠️ 部分 |
| 13 | Agno | ✅ 是 | ✅ 是 | ⚠️ 部分 | ✅ 是 |
| 14 | PraisonAI | ✅ 是 | ✅ 是 | ⚠️ 部分 | ✅ 是 |
| 15 | Dify | ✅ 是 | ⚠️ 部分 | ❌ 否 | ✅ 是 |
| 16 | FastGPT | ⚠️ 部分 | ❌ 否 | ❌ 否 | ✅ 是 |
| 17 | Coze | ✅ 是 | ⚠️ 部分 | ❌ 否 | ✅ 是 |
...
| 能力 | ✅ 完整支持 | ⚠️ 部分支持 | ❌ 不支持 |
|------|-----------|-----------|---------|
| **Agent Loop** | 16/17 | 1/17 (FastGPT) | 0/17 |
| **Multi-Agent 通信** | 8/17 | 6/17 | 3/17 |
| **测试 Harness** | 3/17 | 7/17 | 7/17 |  <!-- ✅ v5.8 核查修正(2026-09-14,本地逐行核对 §7.19.1 矩阵):原写 2/17、8/17 与矩阵矛盾。实际 ✅3(LangChain/LangGraph/PydanticAI)、⚠️7、❌7 -->
| **Prompt 管理** | 11/17 | 5/17 | 1/17 |

**关键发现:**
- **Agent Loop 支持最普遍**:16/17 原生支持,仅 FastGPT 为"部分"(因其侧重 RAG/知识库而非自主 Agent)
- **Multi-Agent 通信分化明显**:原生支持仅 8/17——LangGraph、CrewAI、AutoGen、AgentScope、MetaGPT、OpenAI Swarm、Agno、PraisonAI
- **测试 Harness 是最稀缺的能力**:完整支持仅 LangChain/LangGraph(通过 LangSmith)和 PydanticAI(内置 Evals)
- **Prompt 管理两极分化**:完整支持 11/17,主要集中在有 YAML 配置或可视化界面的框架

#### 7.19.3 逐框架能力说明

| 框架 | Agent Loop 说明 | Multi-Agent 说明 | 测试说明 | Prompt 说明 |
|------|----------------|-----------------|---------|------------|
| **LangChain** | `AgentExecutor` 原生循环 | 需借助 LangGraph 图结构 | LangSmith(tracing+eval) | `ChatPromptTemplate` + PromptHub |
| **LangGraph** | 有向状态图,循环/重试/分支 | Supervisor 模式 + 状态图消息传递 | LangSmith 集成 | 复用 LangChain 模板体系 |
| **AutoGPT** | 自主目标驱动循环 | 单 Agent 设计,不支持 | 无 | 配置文件定义行为提示 |
| **BabyAGI** | 任务创建→排序→执行循环 | 单 Agent,不支持 | 无 | 硬编码在脚本中 |
| **CrewAI** | Agent+Task+ Crew 循环 | 顺序/层级两种编排 | verbose 模式查看日志 | YAML 配置 role/goal/backstory |
| **AutoGen** | 对话式协作循环 | Selector/RoundRobin/GraphFlow | v0.4+ 有评估监控 | `system_message` 控制行为 |
| **AgentScope** | `reply` 方法处理消息 | DialogAgent + UserAgent 交互 | 运行时监控 | 代码中定义,无管理中心 |
| **MetaGPT** | think-act + SOP 驱动 | 模拟软件团队角色协作 | 无 | 每角色内置结构化模板 |
| **OpenAI Swarm** | 工具调用循环 | Handoff 交接机制 | 无(教学项目) | `instructions` 参数定义 |
| **LlamaIndex** | `AgentRunner` + Worker | 数据框架为主,多 Agent 非重点 | 集成 Ragas 评估 | 内置 Prompt 模板系统 |
| **PydanticAI** | 循环 + 自我反思重试 | 支持 A2A 协议、Agent 委派 | **内置强大 Evals 框架** | 静态/动态 instructions + YAML Spec |
| **smolagents** | CodeAgent 生成执行代码 | `ManagedAgent` 多 Agent | 轻量级基础评估 | 基础模板,极简设计 |
| **Agno** | 原生循环 + 记忆 + 知识库 | 全栈多 Agent 平台 | 监控和会话管理 | instructions + 会话管理 |
| **PraisonAI** | 循环 + Self-Reflection | `AgentTeam` + `AgentFlow` | Self-Reflection 自我评估 | YAML 配置 `agents.yaml` |
| **Dify** | 工作流 + Agent 节点(ReAct) | 流程驱动节点传递(非自由通信) | 无(有日志标注) | 可视化提示词管理界面 |
| **FastGPT** | 可视化 Flow 编排(有限) | 单工作流管线,不支持 | 无 | 可视化提示词配置 |
| **Coze** | Bot + 工作流 + 插件 | 多 Bot 工作流串联 | 无 | 可视化人设 + 模板管理 |

#### 7.19.4 按能力需求选型

```
需要 Agent Loop?
├─ 是 → 除 FastGPT 外几乎所有框架都支持
└─ 极致控制流? → LangGraph(状态图 + 条件边 + 循环)

需要 Multi-Agent 通信?
├─ 角色协作 → CrewAI / MetaGPT(角色 + SOP)
├─ 对话式协作 → AutoGen(Selector/RoundRobin)
├─ 分布式多 Agent → AgentScope / Agno
├─ 轻量交接 → OpenAI Swarm(Handoff)
└─ 工作流编排 → PraisonAI / Dify

需要 Testing Harness?
├─ 完整评估体系 → LangChain/LangGraph + LangSmith
├─ 内置 Evals → PydanticAI(Pydantic Logfire 监控)
└─ 自建评估 → 配合 Ragas / Braintrust / DeepEval

需要 Prompt 管理?
├─ YAML 配置驱动 → CrewAI / PraisonAI
├─ 可视化管理 → Dify / Coze / FastGPT
├─ 代码模板系统 → LangChain / LlamaIndex
└─ 类型安全 + YAML Spec → PydanticAI
```
