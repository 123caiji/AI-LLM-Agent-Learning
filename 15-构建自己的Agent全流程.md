...
|------|-----------|-----------|-----------|-----------|-------------|-----------|
| **LangChain** | 链式(Chains)+Agent+Memory 组件化 | 中:Memory 抽象,需自行持久化 | 支持 MCP 适配;工具即函数 | 强:LangSmith trace | 高:GitHub 11.8万 Star,事实行业标准 | 强:任意模型 |
| **Dify** | 可视化画布(图)+低代码工作流 | 平台托管 | 平台插件市场+MCP 支持;强大RAG引擎 | 平台内置日志/分析/追踪 | 高:GitHub 11.7万 Star,支持私有化部署 | 平台内置模型列表 |
...
| 框架 | 适用场景 | 主要坑 |
|------|---------|--------|
| LangChain | 需要深度定制、对灵活性要求高的专业开发者和企业级应用 | 学习曲线陡峭、抽象层次过高、API 变动频繁 |
| Dify | 需要可视化编排的非工程师/企业内部工具/需要私有化部署 | 平台托管的迁移成本、复杂逻辑撞天花板 |
...
### 4.0 生产环境铁律：原型思维 vs 工程纪律

Bandara et al. (2025) 对生产环境中部署 Agent 的实践者进行系统研究后发现一个反直觉结论：**实践者会"故意牺牲 Agent 额外能力以换取可靠性"**。这不是 LLM 能力不足的临时妥协，而是生产级系统与原型系统的本质分野。

**核心区别：**

| 维度 | 原型 Agent | 生产 Agent |
|------|-----------|-----------|
| 优化目标 | "能力展示"——让 demo 看起来厉害 | "一致性执行"——每次运行结果可预期 |
| 工具策略 | 单 Agent 挂多个工具，灵活但难调试 | 单工具单职责 Agent，即使需要更多编排代码 |
| 工具调用的性质 | 允许隐式副作用和状态携带 | 工具调用应为纯函数（同输入→同输出，无隐式副作用） |
| 工作流 | 依赖模型自主规划步骤 | 工作流分解为确定性序列，各步骤有显式错误边界 |

**三条关键实践：**

1. **单工具单职责 Agent**：宁可多写编排代码，也不让一个 Agent 承载多个工具。当多工具 Agent 出现异常时，需要解析 LLM 推理轨迹来定位"哪个工具出了什么问题"；单工具 Agent 的失败直接可见。
2. **工具调用应为纯函数**：同一输入在同一工作流内多次调用必须返回相同结果，无隐式状态携带。纯函数让测试可行、回滚安全、重试可预期。如果 `get_customer_data(customer_id)` 在同一工作流内两次调用返回不同结果，架构就是有问题的。
3. **工作流分解为确定性序列**：复杂任务不应是"跑这个 Agent"，而应是确定的步骤序列——每步都是有限的 Agent 调用，每个边界都是可观察状态、可独立处理失败的检查点。

**经验法则：** 原型优化"能力展示"，生产优化"一致性执行"。两者不是同一系统的不同阶段，而是根本不同的设计哲学。
...
**有界自治（Bounded Autonomy）**

生产环境中的 Agent 不应拥有"无监督执行"的自由。有界自治通过四重显式约束将 Agent 的能力框定在安全边界内：

| 约束 | 含义 | 实现方式 |
|------|------|---------|
| 最大迭代次数上限 | 防止 Loop 失控空转烧 token | `MAX_STEPS=25`（4.3 节已落地） |
| 工具白名单 | Agent 只能调用显式注册的工具，不可动态扩展 | 工具注册表 + 运行时校验 |
| 成本断路器 | 单次任务 token 消耗或 API 调用次数超过阈值自动终止 | 在 model adapter 层累加计数，超限抛异常 |
| 人工升级路径 | 遇到无法处理的异常时，不自行"编造"解决方案，而是暂停并请求人类介入 | LangGraph `interrupt()` + 审批门（11.2 节） |

四条约束缺一不可：缺失任何一条，Agent 就可能在无人值守时造成不可预期的后果。

**容器化部署原则（引用 Mindflow 实践）**

Mindflow 团队的生产经验表明：**从 Day 1 就容器化**，不仅是隔离，更保证环境可复现。具体实践：

- 每个 Agent 服务独立打包为 Docker 镜像，依赖版本锁死
- 通过 `docker-compose` 或 Kubernetes 管理多 Agent 服务的编排与网络
- 工具执行在独立沙箱容器中运行，与 Agent 主进程隔离
- 环境变量注入模型密钥与配置，不硬编码

容器化的核心价值不在"隔离"（那是副产品），而在**可复现**——开发环境、CI 环境、生产环境跑的是完全相同的镜像，不会出现"我机器上能跑"的问题。

**KISS 原则**

> 宁可多写编排代码，不让单体 Agent 承载过多复杂度。

当一个 Agent 的工具数量、Prompt 长度、或推理步数持续增长时，优先考虑拆分为多个单工具 Agent + 确定性编排，而非继续在单体 Agent 内堆砌能力。复杂度分散到编排层（代码、可测试、可版本化），比集中在 Agent 推理层（黑盒、难测试、依赖模型行为）可控得多。
...
| 11 | **单Agent多工具连锁故障** | 一个工具返回异常数据,后续工具基于错误数据继续操作,最终结果彻底偏离 | 多工具Agent的失败级联 | 拆分为单工具Agent+确定性编排 |
...
- Bandara et al. "Production AI Agent Reliability" (2025) — 9项生产级Agent工程实践，系统研究生产环境中实践者如何通过牺牲部分Agent能力换取可靠性
- "国内外主流AI Agent开发框架与平台深度解析" (53ai.com, 2026) — 框架对比与国产平台分析，涵盖LangChain/LangGraph/AutoGen/CrewAI/Dify/FastGPT/Coze等
- "LangGraph vs CrewAI vs AutoGen 完全指南" (meta-intelligence.tech, 2025) — 三框架深度对比与代码示例，含Google Colab可运行实例
