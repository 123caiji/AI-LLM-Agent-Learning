...
| **通用 Agent 构建框架** | **Agno / PraisonAI / smolagents** | **自建 / VPS / 云** | **从零搭建定制 Agent:提供工具/记忆/MCP 等全套积木,自由组装** | 03 章 §X / 12 章 |

> **一句话区分:** 云端任务 Agent 是"外包钟点工"(在别人的地盘干活,干完就走);编程 Agent 是"驻场程序员"(只干代码);工作流平台是"流水线"(步骤你预先画好);**个人常驻 Agent 是"住家管家"**——住在你家(你的机器),认识你家每样东西,记得你所有习惯,随叫随到,有时不叫也到(定时任务);**通用 Agent 构建框架是"建材市场"**——给你工具/记忆/MCP/多模型等全套积木,你自己设计蓝图、自己盖房子。住家,正是它魅力与风险的共同来源。
...
### 6.5 自建 vs 开箱:Agno/PraisonAI 等通用 Agent 框架的定位

OpenClaw 和 Hermes 是"开箱即用"的个人管家——装上就能用,入口在聊天 App。但如果你需要的是**高度定制**(非聊天入口、自定义工作流、特定行业逻辑),还有另一条路:用通用 Agent 框架从零搭建。2026 年最活跃的通用框架中,以下三个尤其适合自托管场景:

| 框架 | 一句话 | 自托管友好度 | 与 OpenClaw/Hermes 的关系 |
|------|------|:---:|------|
| **Agno**(原 Phidata) | 全栈 Agent 平台:AgentOS+记忆+知识库+UI,开箱即用 | ★★★★★ | 互补:你可以用 Agno 搭建一个"自己的 Hermes"——有记忆、有工具、有调度,但入口和形态完全由你定义 |
| **PraisonAI** | 低代码 Agent 框架:5 行起、YAML 配置、100+ 工具 | ★★★★☆ | 互补:更快地从零到一,适合原型验证;但缺少 Hermes 的学习循环和 OpenClaw 的全渠道网关 |
| **smolagents** | 极简 code-act 框架:~1k 行核心,HuggingFace 生态 | ★★★☆☆ | 教学/轻量首选:代码即动作,透明到可以逐行讲解;但不适合做 7×24 常驻管家 |

**什么时候该走"自建"路线而非"开箱":**

| 你的情况 | 建议 | 理由 |
|---------|------|------|
| 需要非聊天入口(如 API 触发、Webhook、自定义 UI) | 自建(Agno/PraisonAI) | OpenClaw/Hermes 的聊天入口是核心假设,绕不开 |
| 要嵌入已有产品/系统 | 自建(Agno/PraisonAI) | 通用框架提供 SDK/API,天然适合嵌入 |
| 只是想"有个 AI 帮我干活" | 开箱(OpenClaw/Hermes) | 自建要写胶水代码,开箱即用省 90% 时间 |
| 想学习 Agent 原理 | 自建(smolagents 入门) → 开箱(深入) | smolagents 代码量极小,读完源码就能理解 Agent Loop 本质;再去看 OpenClaw/Hermes 的工程化实现 |

> **核心判断:** OpenClaw/Hermes 是"产品",Agno/PraisonAI/smolagents 是"框架"。前者给你一个家,后者给你一堆建材。两种选择都合理——关键是先想清楚你要的是"住进去"还是"盖房子"。
...
| 通用 Agent 框架 | Agno(原 Phidata):github.com/agno-agi/agno;PraisonAI:github.com/MervinPraison/PraisonAI;smolagents:github.com/huggingface/smolagents |
...
7. **"开箱"与"自建"两条路:** OpenClaw/Hermes 是产品(开箱即用),Agno/PraisonAI/smolagents 是框架(从零搭建);前者省时间,后者给自由度——先想清楚"住进去"还是"盖房子",再选工具(§6.5)。
