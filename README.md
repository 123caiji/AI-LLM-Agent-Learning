...
> 全套合计 **~26,520 行** Markdown,涵盖 **536 速查术语**、**610+ 表格/图示**、**620+ 代码/命令/配置示例块**。
...
| **合计** | **~26,520** | **536(速查表)** | **610+ 表格** | **620+ 代码块** | ✅ |

### 15.2 版本日志

#### v4.7(2026-07-28 第十章部署运维量化迭代:GPU 分区 + 指标量化 + 缓存命中率)

- ✅ 第十章《部署网关运维》新增 **§9.6 GPU 分区与虚拟化**:
  - 三种分区方案深度对比:**MIG**(硬件级,A100/H100/Blackwell 最多 7 实例)、**MPS**(软件级,并行共享)、**Time-Slicing**(默认,轮转)
  - MIG 实例规格表(A100 80GB / Blackwell GB200)、MPS 配置命令、K8s Time-Slicing 配置
  - 10 维度对比表(隔离/显存/故障/QoS/切换/动态/LoRA/GPU/实例数/兼容) + 选型决策树
- ✅ §16.3 关键指标详解从 2 节扩展到 **5 节**,全面量化:
  - §16.3.1 延迟指标:新增 ITL(Inter-Token Latency)与 Queue Time;延迟公式与分解;TTFT 量化基准表(8 种模型/GPU/Prompt 组合);ITL/TPOT 量化基准表 + 用户体验阈值
  - §16.3.2 吞吐指标:TPS 量化基准表(6 种配置);TPS 公式与提升路径
  - §16.3.3 错误率指标(新增):7 种错误类型 × HTTP 状态码 × SLO 目标;错误率计算公式;Prometheus 告警规则
  - §16.3.4 SLI/SLO/SLA 量化框架(新增):层级定义 + LLM 服务 SLO 模板(8 项指标)+ Error Budget 计算
  - §16.3.5 指标采集命令行验证(新增):curl /metrics + vLLM benchmark_serving 压测示例
- ✅ §17 缓存新增 **§17.5 缓存命中率量化分析**:
  - 各层缓存命中率基准表(L1-L5,5 层 × 命中率/延迟降低/成本降低/场景)
  - 命中率计算公式(Prefix/Prompt/Semantic 三种)+ 缓存收益量化(延迟/成本)
  - vLLM Prefix Cache 命中率监控(Python + PromQL + Grafana 告警)
  - 命中率优化策略表(7 项)+ Prompt 结构优化示例
- ✅ 词汇表从 50 条扩展到 **63 条**,新增 MIG/MPS/Time-Slicing/QoS/ITL/Goodput/SLI/SLO/SLA/Error Budget/Cache Hit Rate/DCGM/Noisy Neighbor
- ✅ §19.3 性能调优清单新增 GPU 分区、TTFT/ITL 监控、错误率监控、缓存命中率监控 4 项
- ✅ §21.2 成本优化表新增 GPU 分区(MIG)策略,Prefix Cache/Semantic Cache 补充命中率量化数据
- ✅ README 统计更新:合计 ~26,520 行、536 速查术语
