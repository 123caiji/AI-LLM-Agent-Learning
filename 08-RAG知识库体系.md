...
### 5.3.1 HippoRAG 2(海马体 RAG v2,2025)

> v5.4 新增(2026-08-03)

**出品:** 俄亥俄州立大学(OSU-NLP),2025 年发布
**GitHub:** https://github.com/OSU-NLP-Group/HippoRAG
**定位:** HippoRAG 的全面升级版,在所有基准类别上超越 GraphRAG / LightRAG / HippoRAG / RAPTOR,更接近真正的长期记忆系统。

**核心改进(相比 HippoRAG v1):**

| 维度 | HippoRAG v1 (2024) | HippoRAG 2 (2025) |
|------|---------------------|---------------------|
| **检索算法** | 个性化 PageRank (PPR) | PPR + 深度段落整合 |
| **LLM 参与度** | 主要在索引阶段 | 检索阶段也增强 LLM 参与(在线检索中更有效地使用) |
| **知识图谱** | OpenIE 三元组 | 开放知识图谱(更灵活的图结构) |
| **多跳推理** | 强 | 更强(段落整合优化多跳路径) |
| **持续学习** | 支持 | 增强(新知识增量融入) |
| **整体性能** | 超越 Naive RAG | 超越 GraphRAG / LightRAG / RAPTOR |
...
│                                                   │
│  协同复制人类长期记忆中的交互作用:                   │
│  新皮层(LLM)↔ 旁海马区(编码)↔ 海马体(图谱记忆)    │
└─────────────────────────────────────────────────┘
```

**与 GraphRAG / LightRAG 的关键区别:**
- **GraphRAG:** 依赖社区发现 + 社区摘要,全局总结强但构建成本高
- **LightRAG:** 双层检索(实体+主题),轻量但多跳推理弱于 HippoRAG 2
- **HippoRAG 2:** PPR 算法天然适合沿关系链追踪到精确事实,多跳推理最强;但复杂推理场景优势缩小
